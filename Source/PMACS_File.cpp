...
//the record for comparison, the file index, line number
bool correctRecordLength(string record, int fileIndex, int lineNumber)
{
	if (record.length() == recordLength[fileIndex])
	{
		return true;
	}
	//compose error message
	string message = "Record length mismatch in file: ";
	message += fileName[fileIndex];
	message += " on line: ";
	message += to_string(lineNumber);
	message += ". Expected: ";
	message += to_string(recordLength[fileIndex]);
	message += " | Actual: ";
	message += to_string(record.length());
	Plog.logError("RecordLengthCheck", message);
	return false;
}